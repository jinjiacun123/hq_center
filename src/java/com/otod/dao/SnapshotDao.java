...
public class SnapshotDao extends ParentDao {

    public List<KLineData> getBySymbol(String symbol, int num) {
        List<KLineData> list = new ArrayList<KLineData>();
        String sql = "select * from snapshot where symbol=? order by quote_date desc limit ? ";
    ...
    public void save(KLineData data) {
        String sql = "insert into snapshot (symbol) values(?)";
        PreparedStatement ps = null;
        Connection con = null;
        try {
            con = getConnector().getConnection();
