...
public class Minute15Dao extends ParentDao {

    public List<KLineData> getBySymbol(String symbol, int count) {
        List<KLineData> list = new ArrayList<KLineData>();
        String sql = "select  * from minute15 where symbol=? order by quote_date desc,quote_time desc limit ? ";
    ...
    public List<KLineData> getBySymbol(String symbol, int startDate, int endDate) {
        List<KLineData> list = new ArrayList<KLineData>();
        String sql = "select * from minute15 where symbol=? and quote_date>=? and quote_date<=? order by quote_date desc,quote_time desc";
        PreparedStatement ps = null;
        ResultSet rs = null;
        Connection con = null;
        try {
    ...
    public void save(KLineData data) {
        String sql = "insert into minute15 (symbol,quote_date,quote_time,open_price,high_price,low_price,close_price,volume,turnover) values(?,?,?,?,?,?,?,?,?)";
        PreparedStatement ps = null;
        Connection con = null;
        try {
            con = getConnector().getConnection();
    ...
    public void update(KLineData data) {
        String sql = "update minute15 set open_price=?,high_price=?,low_price=?,close_price=?,volume=?,turnover=? where symbol=? and quote_date=? and quote_time=?";
        PreparedStatement ps = null;
        Connection con = null;
        try {
            con = getConnector().getConnection();
