...
public final class Utils {
	...
	byte paddingByte) throws java.io.UnsupportedEncodingException {
		...
		switch( alignment) {
			...
			case ALIGN_LEFT:
				System.arraycopy( text.getBytes( characterSetName), 0, byte_array, 0,  text.getBytes(characterSetName).length);
				break;

			case ALIGN_RIGHT:
				int t_offset = length - text.length();
				System.arraycopy( text.getBytes( characterSetName), 0, byte_array, t_offset, text.getBytes(characterSetName).length);
				break;
			}	
