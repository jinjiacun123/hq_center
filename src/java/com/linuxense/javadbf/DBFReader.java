 ...
 *
 * <p>
 * Fetching rocord is possible only in the forward direction and cannot be
                    ...
                    case 'N':
                        try {
                            ...
                            if (t_numeric.length > 0 && !Utils.contains(t_numeric, (byte) '?')) {
                                String str = new String(t_numeric);
                                str = str.replaceAll("\0", "");// \0是c++空字符
                                if (str.equals("")) {
                                    recordObjects[i] = 0;
                                } else {
                                    str = str.split(" ")[0];
                                }
                                recordObjects[i] = new Double(str);
                            } else {
                                recordObjects[i] = 0d;
                            }
                        } catch (NumberFormatException e) {
